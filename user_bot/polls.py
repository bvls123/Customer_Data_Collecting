...
class PersonalData:
    ...
    def question_7(self, message):
        ...
        self.bot.send_message(user_id, 'Вкажіть область в якій проживаєте ⬇️', reply_markup=regions_markup(regions))
    ...
    def question_8(self, message):
        ...
        self.bot.send_message(user_id, 'Вкажіть район в якому проживаєте ⬇️', reply_markup=cities_markup(cities))
...
class ProductPoll:
    ...
    def poll_acception(self):
        name = get_user_name(self.telegram_id)
        message = self.bot.send_message(self.telegram_id, f'{name}, нас є коротке опитування про вашу останню покупку 🛍️.', reply_markup=poll_accept())
        self.bot.register_next_step_handler(message, self.poll_acception_processing)
...
class RefundPoll:
    ...
    def poll_acception(self):
        name = get_user_name(self.telegram_id)
        message = self.bot.send_message(self.telegram_id, f'{name}, нещодавно ви скористалися послугою повернення ↩️, у нас є коротку опитування про це.', reply_markup=poll_accept())
        self.bot.register_next_step_handler(message, self.poll_acception_processing)
...
class RepairPoll:
    ...
    def poll_acception(self):
        name = get_user_name(self.telegram_id)
        message = self.bot.send_message(self.telegram_id, f'{name}, нещодавно ви користувалися послугами нашого сервісного центру 🛠️, у нас є коротке опитування про це.', reply_markup=poll_accept())
        self.bot.register_next_step_handler(message, self.poll_acception_processing)
